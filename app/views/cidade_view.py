...
class Cidade_View:
    ...
    def preencher_estados(self, estados):

        self.estados = estados

        self.cbo_estado["values"] = [
            f"{estado.id} - {estado.sigla} - {estado.nome}"
    ...
    def preencher_campos(self, cidade):
        ...
        self.txt_id.insert(0, str(cidade.id))
        ...
        estado_texto = (
            ...
            f"{cidade.estado.nome}"
        )

        self.cbo_estado.set(estado_texto)
    ...
    def ler_dados_cidade(self):
        ...
        if self.cbo_estado.get():
            estado_id = self.cbo_estado.get().split(" - ")[0]
